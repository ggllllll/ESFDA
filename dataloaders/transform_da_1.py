...
def w_augmentation():
    w_transforms = []
    w_transforms.append(BrightnessMultiplicativeTransform((0.7, 1.5), per_channel=True, p_per_sample=0.5))
    w_transforms.append(GammaTransform(gamma_range=(0.5, 2), invert_image=False, per_channel=True, p_per_sample=0.5))
    # w_transforms.append(GammaTransform(gamma_range=(0.5, 2), invert_image=True, per_channel=True, p_per_sample=0.15))
    w_transforms.append(GaussianNoiseTransform(noise_variance=(0, 0.05), p_per_sample=0.5))
    w_transforms.append(GaussianBlurTransform(blur_sigma=(0.5, 1.5), different_sigma_per_channel=True,
                                               p_per_channel=0.5, p_per_sample=0.5))
    w_transforms = Compose(w_transforms)
    return w_transforms
...
def source_collate_fn_tr_fda(batch):
    ...
    tr_transforms = get_train_transform()
    data_dict = tr_transforms(**data_dict)
    w_transforms = w_augmentation()
    w_dict = {'data': data_dict['data'].copy()}
    w_dict = w_transforms(**w_dict)
    data_dict['fda_data'] = w_dict['data']
    return data_dict

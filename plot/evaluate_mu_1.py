...
def eval_print_all_CU(pred_img_path, gt_img_path):
    ...
    for i in range(n):
        ...
        pred_name = os.path.join(pred_img_path,temp_list[:-4]+'.tif') #获取伪标签
        gt_name = os.path.join(gt_img_path,temp_list[:-4]+'.tif')  #获取真实标签
        ...
        # 画图
        gt_oc = cv2.resize(cv2.imread(gt_name, 0),(512,512), interpolation=cv2.INTER_NEAREST)   #真实标签oc
        gt_oc[gt_oc == 0] = 255
        gt_oc[gt_oc == 128] = 0

        gt_od = cv2.resize(cv2.imread(gt_name, 0),(512,512), interpolation=cv2.INTER_NEAREST)   #真实标签od
        gt_od[gt_od == 128] = 255
        gt_od[gt_od == 0] = 1
        gt_od[gt_od == 255] = 0
        gt_od[gt_od == 1] = 255

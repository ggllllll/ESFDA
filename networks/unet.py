...
class SaveFeatures():
    ...
    def hook_fn(self, module, input, output): self.features = output          #钩子函数，可以获取中间层的输出，用于提取特征，进行可视化
...
class UnetBlock(nn.Module):
    ...
    def forward(self, up_p, x_p):
        up_p = self.tr_conv(up_p)        #对上采样得到的特征图进行转置卷积操作
        x_p = self.x_conv(x_p)           #对下采样得到的特征图进行卷积操作
        cat_p = torch.cat([up_p, x_p], dim=1)       #将上采样得到的特征图和下采样得到的特务征途进行拼接
...
class UNet(nn.Module):
    def __init__(self, resnet='resnet34', num_classes=2, pretrained=False, is_embed=False):
        ...
        self.global_features = SaveFeatures(self.rn[-1])     #将之前保存的特征中最后一层作为全局特征进行保存
        self.decoder_features = [SaveFeatures(i) for i in [self.up1, self.up2, self.up3, self.up4]]   #将self.up1, self.up2, self.up3, self.up4这几个层作为解码器的特征进行保存
    ...
    def forward(self, x, rfeat=False, mfeat=False):
        x = F.relu(self.rn(x))  #对输入的数据进行处理
        x = self.up1(x, self.sfs[3].features)    #对输入的数据进行上采样操作
